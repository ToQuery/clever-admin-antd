...
// 可控长度的随机数拼接时间戳成产唯一id
const createUniqID = (length: number) : string => Number(Math.random().toString().substr(3,length) + Date.now()).toString(36);

export type DragModalProps = {
  id: () => string;
  dragWrapDom: HTMLElement;// 包裹拖拽元素的元素
  dragDom: HTMLElement; // 拖拽的目标元素，在Modal组件中对应class为.antd-modal的元素
  dragging: false; // 是否拖拽的状态标志
  tLeft: 0; //
  tTop: 0;  //   坐标轴
  //modal的左上角相对于屏幕左上角的偏移，也是拖拽原点
  points: [0, 0]; // 拖拽原点
  rect:  [0, 0, 0, 0] ;// 记录可视区宽高和拖拽元素宽高
  onMouseDown: Function;
  onMouseUp: Function;
  onMouseMove: Function;
  onContentMoseDown: Function;
};
